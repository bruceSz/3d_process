...
Description: 用于根据kitti点云，相机矫正文件以及左眼图片来生成用于训练pointnet的数据集
FilePath: /point_process_piplines/generate_training_set.py
'''
import argparse
import glob
import os
...
def get_object_pcd_df(pcd, idx, num_sample):
    ...
    # N = np.asarray(pcd.points)[idx].shape[0]

    # if N < num_sample:
    #     points = np.asarray(pcd.points)[idx]
    #     normals = np.asarray(pcd.normals)[idx]
    #     points = np.pad(points,((0,num_sample-N),(0,0)),mode="constant",constant_values=(0,0))
    #     normals = np.pad(normals,((0,num_sample-N),(0,0)),mode="constant",constant_values=(0,0))

    # else:
    #     pcd_new = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(np.asarray(pcd.points)[idx]))
    #     pcd_new.normals = o3d.utility.Vector3dVector(np.asarray(pcd.normals)[idx])

    #     point_cloud = PyntCloud.from_instance("open3d",pcd_new, mesh=False)
    #     point_cloud = point_cloud.get_sample("points_random",n=num_sample,as_PyntCloud=True)
    #     pcd_new = point_cloud.to_instance("open3d",mesh = False)
    #     points = np.asarray(pcd_new.points)
    #     normals = np.asarray(pcd_new.normals)


    #print(np.concatenate((points,normals),axis=1))
...
def get_args():
    ...
    parser.add_argument("--output_dir",type=str,default="/mnt/e/download/kitti/class_train")
...
if __name__ == "__main__":
    args = get_args()
    label_input_dir = "/mnt/e/download/kitti/data_object_label_2/training/label_2"
    # left color
    img_input_dir = "/mnt/e/download/kitti/data_object_image_2/training/image_2"
    calib_input_dir =  "/mnt/e/download/kitti/data_object_calib/training/calib"
    velo_input_dir = "/mnt/e/download/kitti/data_object_velodyne/training/velodyne"
    
    output_dir = args.output_dir
    max_distance = args.max_distance

    # 获取label文件数量
    N = len(glob.glob(os.path.join(label_input_dir,"*.txt")))
    ...
    for index in progres(range(N)):
        ...
        os.path.join(velo_input_dir, f'{index:06d}.bin'))
        # 读取对应的相机矫正文件以及转换矩阵
        param = read_calib(
        os.path.join(calib_input_dir,  f'{index:06d}.txt'))

        # 读取label文件
        df_label = read_label(
        os.path.join(label_input_dir,  f'{index:06d}.txt'),param)
        ...
        for idx, label in df_label.iterrows():
            ...
            if (k > 0):     
                ...
                # if category == 'vehicle':
                #     points_vechile = np.concatenate((points_vechile,np.asarray(segmented_objects.points)[idx_object]),axis=0)
                # elif category == 'pedestrian':
                #     points_pedestrian = np.concatenate((points_pedestrian,np.asarray(segmented_objects.points)[idx_object]),axis=0)
                # elif category == 'cyclist':
                #     points_cyclist = np.concatenate((points_cyclist,np.asarray(segmented_objects.points)[idx_object]),axis=0)
