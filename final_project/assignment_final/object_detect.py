'''
Author: your name
Date: 2021-05-10 11:29:49
LastEditTime: 2021-05-10 22:58:34
LastEditors: Please set LastEditors
Description: In User Settings Edit
FilePath: /point_process_piplines/object_detect.py
'''
import os
import glob
import argparse
...
def preprocess(
    ...
    assert(len(X) % config["batch_size"] == 0)
    dat = []
    for idx in range(0,len(X), config["batch_size"]):
        dat.append((X[idx: idx+config["batch_size"]], y[idx: idx+config["batch_size"]]))
...
def get_args():
    parser = argparse.ArgumentParser("classification after cluster")
    parser.add_argument("--input_dir",type=str,default="/mnt/e/download/kitti/class_train_aug")
...
def detect(
    ...
    # 设置路径:
    input_velodyne = os.path.join(velo_input_dir,  f'{index:06d}.bin')
    input_params = os.path.join(calib_input_dir,  f'{index:06d}.txt')
    output_label = os.path.join("train_obj_det_output", f'{index:06d}.txt')
...
if __name__ == "__main__":
    ...
    for label in prog(
        glob.glob(
            os.path.join(label_input_dir, '*.txt')
        )
    ):
        # 获取索引:
        index = int(
